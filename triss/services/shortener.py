...
`created_at` timestamp. The shortener is pointed at OUR OWN
`{PUBLIC_BASE_URL}/v/<session_id>` landing route (triss/web/server.py) —
never directly at a Telegram deep link, and with no query string to
survive the provider's redirect chain. Only once that route is actually
reached (`handle_landing()`) do we:

  1. Check `elapsed = now - created_at` against [Minimum Time, Maximum
     Time]; too fast -> BYPASS, too slow -> EXPIRED.
  2. If within window, mint a random single-use proof for the first time
     (CREATED -> REDIRECTED), and 302-redirect the browser into
     `https://t.me/<bot>?start=verify_<session_id>.<proof>`.

When Telegram then delivers that `/start verify_...` update
(`evaluate_verification()`), we only check that the `(session_id, proof)`
pair matches exactly one REDIRECTED session (a guessed/forged/replayed
payload is rejected outright) and move REDIRECTED -> VERIFIED. Timing is
irrelevant at this step — it was already enforced at the landing hit.

This still cannot distinguish "the user genuinely sat through the
shortener's page" from "a script that reaches our landing route directly
and waits out the Minimum Time" — reaching our own route isn't proof of
having watched the shortener's ad/page either. It's a deliberate,
...
for a different Telegram user, tampering with the proof, concurrent/
duplicate delivery of the same session, and (new) the shortener provider
corrupting a long, parameter-heavy destination URL, since the only URL
ever handed to the provider is the short, query-string-free landing path.
...
module and every caller must keep describing the current flow as
time-window gating, never as "verified" in the sense of proven
completion.
--------------------------------------------------------------------------
"""
...
# Deploy-verification marker — proves at a glance (via Railway startup logs)
# which build of this module is actually running, ending any ambiguity
# about whether a given fix has been deployed. Bump the date/tag whenever
# this file changes in a way that matters for debugging.
logger.warning("triss.shortener module loaded — build marker: SHORTENER-LANDING-FIX-2026-08-14-02")
...
class SessionState:
    CREATED = "created"        # session minted, shortener link (to our own /v/<id>) handed out
    REDIRECTED = "redirected"  # /v/<id> was reached within [minimum_time, maximum_time] of
                                # creation, proof was minted just now, browser sent into Telegram
    VERIFIED = "verified"      # correct proof presented for a REDIRECTED session
    CONSUMED = "consumed"      # VERIFIED session has been used to deliver content (terminal)
    BYPASS = "bypass"          # /v/<id> reached faster than minimum_time (terminal)
    EXPIRED = "expired"        # /v/<id> reached after maximum_time (terminal)
...
class VerificationOutcome:
    """Outcomes of evaluate_verification() — the `/start verify_...` step.
    BYPASS/EXPIRED are no longer produced here (that timing check now
    happens at the /v/<session_id> landing hit, see LandingOutcome); kept
    defined so any external code referencing them doesn't break."""
    NOT_FOUND = "not_found"
...
class LandingOutcome:
    """Outcomes of handle_landing() — the `/v/<session_id>` web-route hit."""
    NOT_FOUND = "not_found"
    ALREADY_USED = "already_used"
    BYPASS = "bypass"
    EXPIRED = "expired"
...
async def start_new_verification(bot_username: str, user_id: int, access_token: str,
    ...
    """Creates a brand-new CREATED session (no proof minted yet — see
    module docstring) and shortens a link to OUR OWN `/v/<session_id>`
    landing route, never a raw Telegram deep link. Returns (session_doc,
    short_url); short_url is None if PUBLIC_BASE_URL isn't configured or
    the provider call failed — callers must treat that as 'verification
    ...
    if not config.public_base_url:
        # The Shortener enable-toggle (triss/handlers/callbacks.py) is
        # supposed to make this unreachable, but never silently fall back
        # to a raw Telegram deep link if it happens anyway — that's the
        # exact bug this design replaced (provider-mangled long URLs).
        logger.error(
            "Cannot start Shortener verification for session_id=%s: PUBLIC_BASE_URL "
            "is not configured on the server.", session_id,
        )
        return session, None

    landing_url = f"{config.public_base_url}/v/{session_id}"
    # Unconditional diagnostic — logs the exact outgoing long_url every
    # time, success or failure, so "what did we actually try to shorten"
    # is never a guessing game again. Safe to leave in production: this
    # value never contains BOT_TOKEN, API_HASH, MONGO_URI, or the
    # shortener API key — only our own public base URL and a random
    # session id.
    logger.warning(
        "Shortener: generated long_url for session_id=%s bot_username=%r -> long_url=%r",
        session_id, bot_username, landing_url,
    )
...
async def handle_landing(session_id: str) -> tuple[str, Optional[str], Optional[dict]]:
    """Server-side handling of a `/v/<session_id>` hit (triss/web/server.py)
    — the shortener's own redirect chain bouncing the browser onto OUR
    route. Reaching this route within [minimum_time, maximum_time] of
    creation is what's being verified (see module docstring); this is
    also the ONLY place the single-use proof is minted, never at session
    creation. Returns (outcome, deep_link_or_None, session_doc_or_None);
    callers must only redirect the browser when outcome is REDIRECTED."""
    ...
    if session["verification_status"] != SessionState.CREATED:
        # Already redirected (duplicate hit / replay) or already resolved
        # some other way — never mint a second proof for one session.
        return LandingOutcome.ALREADY_USED, None, session
    ...
    # Atomic transition filtered on the current status — the replay/
    # concurrency guard: only the single request that wins this update
    # may mint the proof and redirect. A duplicate/concurrent hit on the
    # same /v/<session_id> finds no matching document and is rejected.
    updated = await db.transition_verification_session(
        session_id, [SessionState.CREATED], SessionState.REDIRECTED,
        extra_set={"proof_hash": proof_hash, "redirected_at": now},
    )
    if updated is None:
        current = await db.get_verification_session(session_id)
        return LandingOutcome.ALREADY_USED, None, current

    bot_username = updated.get("bot_username")
    if not bot_username:
        logger.error("Verification session %s has no bot_username on record.", session_id)
        return LandingOutcome.NOT_FOUND, None, updated
...
async def evaluate_verification(session_id: str, proof: Optional[str]) -> tuple[str, Optional[dict]]:
    ...
    deep link — the browser bounce-back from our own `/v/<session_id>`
    landing route (handle_landing(), above). Trusts nothing from the
    client except the two opaque strings, checked against the stored,
    hashed session record. Timing was already enforced at the landing
    hit, so only the proof and the REDIRECTED status matter here."""
    session = await db.get_verification_session(session_id)
    if session is None:
        return VerificationOutcome.NOT_FOUND, None

    if session["verification_status"] != SessionState.REDIRECTED:
        # Anything other than "freshly redirected, awaiting its one proof"
        # is either already resolved (VERIFIED/CONSUMED) or was never
        # legitimately redirected (CREATED/BYPASS/EXPIRED/INVALID) —
        # never re-evaluable.
    ...
    updated = await db.transition_verification_session(
        session_id, [SessionState.REDIRECTED], SessionState.VERIFIED, extra_set={"completed_at": time.time()}
    )
    if updated is None:
        current = await db.get_verification_session(session_id)
