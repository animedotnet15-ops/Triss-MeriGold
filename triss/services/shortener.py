...
Genuine completion verification would require either routing the
shortener through a landing page we control (so reaching it is itself
proof of having gone through the shortener's redirect), or real
provider-side confirmation (a verification API, signed callback, or
completion token — see `ShortenerProvider.verify_completion()`, kept
here as an unused extension point in case a real provider adapter is
added later). Neither is used by this flow; this module and every
caller must keep describing this as time-window gating, never as
"verified" in the sense of proven completion.
--------------------------------------------------------------------------
"""
...
logger.warning("triss.shortener module loaded — build marker: SHORTENER-DEBUG-2026-08-14-01")
...
async def start_new_verification(bot_username: str, user_id: int, access_token: str,
    ...
    # shortener API key — only the bot's public @username, a random
    # session id, and a single-use proof (both already scoped to a
    # short-lived, hash-verified session).
    logger.warning(
        "Shortener: generated long_url for session_id=%s bot_username=%r -> long_url=%r",
        session_id, bot_username, deep_link,
    )
