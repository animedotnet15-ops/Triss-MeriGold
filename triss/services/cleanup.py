...
1. SessionManager — tracks the OWNER's current multi-step interaction
   (genlink waiting, batch active, broadcast waiting, force-sub add
   flows, store channel setup, welcome setup, auto-delete setup,
   backup/restore confirmation). Sessions live in memory (a single bot
   process owns exactly one active owner session at a time — there is
   only one OWNER_ID by spec) and expire on their own after a timeout
   so a stale flow can never hijack a later command.
