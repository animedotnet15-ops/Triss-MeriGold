...
# ---------------------------------------------------------------------------
# Links (genlink / batch / custombatch)
# ---------------------------------------------------------------------------
...
async def create_verification_session(user_id: int, access_token: str,
    ...
    doc = {
        ...
        "completed_at": None,
        "consumed_at": None,
        # Never store the raw proof — only a salted hash of it, set once the
        # session is legitimately redirected through our own landing route.
        "proof_hash": None,
        ...
        # (tracking id, callback token, etc) it can use to look up or
        # validate this specific session's completion. Unused by the
        # bundled GenericQueryProvider. Never treated as proof by itself —
        # see ShortenerProvider.verify_completion() in
