...
def _shortener_status_text(shortener_settings: dict) -> str:
    ...
    return (
        ...
        f"Maximum: {maximum}s\n"
        f"Tutorial: {tutorial}\n\n"
        "ℹ️ This checks that the browser passed through the shortener and "
        "our own landing page, within the configured time window — it does "
        "not confirm the shortener's page/ad content was actually engaged "
        "with. Treat it as redirect-plus-delay gating, not a completion proof."
    )
    ...
    if action == "setdomain":
        ...
        if not shortener_settings.get("enabled"):
            ...
            if not _config.public_base_url:
                ...
                return
            # NOTE: Shortener verification here is redirect-plus-time-window
            # gating (the shortener must land on our own /v/<session_id>
            # route, then the deep link must be opened within
            # Minimum/Maximum Time) — it is not genuine provider-side
            # completion verification, and no provider capability is
            # required to enable it. See the module docstring in
