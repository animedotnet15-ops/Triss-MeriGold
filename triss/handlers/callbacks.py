...
async def welcome_router(client, cq: CallbackQuery) -> None:
    ...
    if action == "setphoto":
        ...
        if photo_id:
            ...
            await client.send_message(cq.from_user.id, text, disable_web_page_preview=True)
    ...
    if action == "setdomain":
        ...
        if not shortener_settings.get("enabled"):
            ...
            if not _config.public_base_url:
                ...
                return
            # Fail closed: refuse to enable unless the configured provider
            # can genuinely attest completion server-side (verification API,
            # signed callback, completion token, etc). Reaching our own
            # /v/<session_id> landing route and waiting out the timer are
            # NOT completion evidence — see triss/services/shortener.py.
            from triss.services import shortener as shortener_service
            if not shortener_service.provider_supports_verification(shortener_settings):
                # Telegram's answerCallbackQuery text is capped at 200 chars;
                # this explanation is longer, so the short form goes in the
                # alert toast and the full detail is edited into the message
                # body instead (edit_text has no such limit). Sending the
                # long form via cq.answer() raised MESSAGE_TOO_LONG on
                # messages.SetBotCallbackAnswer and crashed this handler.
                await cq.answer(
                    "Provider has no completion verification. Cannot enable — see message.",
                    show_alert=True,
                )
                await _edit(
                    cq,
                    "⚠️ ᴄᴀɴɴᴏᴛ ᴇɴᴀʙʟᴇ sʜᴏʀᴛᴇɴᴇʀ\n\n"
                    "This shortener provider has no genuine server-side completion "
                    "verification (no verification API, signed callback, or completion "
                    "token). Shortener cannot be enabled with it — content would never "
                    "be deliverable. Implement a ShortenerProvider subclass with real "
                    "verify_completion() support for your provider first (see "
