...
def _shortener_status_text(shortener_settings: dict) -> str:
    ...
    provider = shortener_service.get_provider(shortener_settings)
    provider_name = "VPLinks (AdLinkFly-compatible API)" if provider else "Not configured"
    verification = (
        "✅ Available" if shortener_service.provider_supports_verification(shortener_settings)
        else "⚠️ Unavailable (link generation only — see /help)"
    )
    return (
        "🌐 sʜᴏʀᴛᴇɴᴇʀ\n\n"
        f"Status: {status}\n"
        ...
        f"Maximum: {maximum}s\n"
        f"Tutorial: {tutorial}\n"
        f"Completion Verification: {verification}"
    )
    ...
    if action == "setdomain":
        ...
        if not shortener_settings.get("enabled"):
            ...
            if shortener_settings.get("maximum_seconds", 0) < shortener_settings.get("minimum_seconds", 0):
                ...
                return
            from triss.config import config as _config
            if not _config.public_base_url:
                await cq.answer(
                    "PUBLIC_BASE_URL is not configured on the server (required so the "
                    "Shortener can point at this bot's own verification landing page). "
                    "Set it in the environment and restart the bot.",
                    show_alert=True,
                )
                return
            # LINK GENERATION and COMPLETION VERIFICATION are separate
            # capabilities (see triss/services/shortener.py). Enabling only
            # requires the former to genuinely work, proven here with one
            # real, live call to the provider — never a fabricated success.
            from triss.services import shortener as shortener_service
            ok, result = await shortener_service.test_provider(shortener_settings)
            if not ok:
                await cq.answer(
                    "Could not create a real short link with this provider. See message.",
                    show_alert=True,
                )
                await _edit(
                    cq,
                    "⚠️ ᴄᴀɴɴᴏᴛ ᴇɴᴀʙʟᴇ sʜᴏʀᴛᴇɴᴇʀ\n\n"
                    f"The configured provider failed to create a test short link:\n`{result}`\n\n"
                    "Check the Domain and API key and try again.",
                    shortener_menu(False),
                )
                return
            new_value = True
            await db.update_settings({"shortener.enabled": new_value})
            settings = await db.get_settings()
            # Telegram caps answerCallbackQuery text at 200 chars (a longer
            # string here raises MESSAGE_TOO_LONG on messages.SetBotCallbackAnswer),
            # so the short form goes in the toast and the full detail
            # (including the completion-verification caveat, if any) goes
            # into the edited message body instead.
            body = _shortener_status_text(settings["shortener"]) + f"\n\nTest link created: {result}"
            if not shortener_service.provider_supports_verification(shortener_settings):
                # Fail closed remains in force downstream: this provider has
                # no genuine server-side completion check (no verification
                # API, signed callback, or completion token), so real short
                # links WILL be generated and shown to users, but
                # evaluate_verification() can never mark a session VERIFIED
                # through it — reaching our own /v/<id> redirect and waiting
                # out the timer are explicitly not treated as completion
                # evidence. Content will not be delivered through this gate
                # until a provider with genuine completion verification is
                # configured (implement a new ShortenerProvider subclass).
                body += (
                    "\n\n⚠️ This provider has no completion verification, so file "
                    "delivery after the shortener step will not succeed until a "
                    "provider with real verification is configured."
                )
            await _edit(cq, body, shortener_menu(new_value))
            await cq.answer("Shortener enabled.", show_alert=True)
            return
        new_value = False
        await db.update_settings({"shortener.enabled": new_value})
        settings = await db.get_settings()
        await _edit(cq, _shortener_status_text(settings["shortener"]), shortener_menu(new_value))
        await cq.answer("Shortener disabled.", show_alert=True)
        return
    elif action == "test":
        from triss.services import shortener as shortener_service
        if not shortener_settings.get("domain") or not shortener_settings.get("api_key"):
            await cq.answer("Set a Domain and API key first.", show_alert=True)
            return
        await cq.answer("Testing provider...", show_alert=False)
        ok, result = await shortener_service.test_provider(shortener_settings)
        if ok:
            await cq.message.reply_text(f"✅ Test succeeded. Short URL: {result}")
        else:
            await cq.message.reply_text(f"❌ Test failed: {result}")
        return
    await cq.answer()
