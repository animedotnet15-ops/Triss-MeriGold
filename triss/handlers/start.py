...
as Telegram deep links work:
  ...
  /start verify_<session_id>.<proof> -> the user's browser bounced back
                                     from our own /v/<session_id> landing
                                     route (see triss.services.shortener);
                                     the proof is validated server-side
                                     before anything is delivered, flagged
                                     as a bypass, or reported expired
...
def _get_welcome_sticker_ids(welcome: dict) -> list[str]:
    """Returns the configured welcome sticker file_ids, in send order.

    Supports an arbitrary number of stickers via `welcome.sticker_file_ids`
    (list), while remaining backward compatible with the original
    single-sticker `welcome.sticker_file_id` field for settings documents
    that predate multi-sticker support. Either way, nothing is sent unless
    `sticker_enabled` is set.
    """
    if not welcome.get("sticker_enabled"):
        return []
    ids = welcome.get("sticker_file_ids")
    if isinstance(ids, list) and ids:
        return [sticker_id for sticker_id in ids if sticker_id]
    single = welcome.get("sticker_file_id")
...
async def _send_welcome(message: Message, settings: dict) -> None:
    ...
    # 1) STICKERS - sent first, sequentially. Missing config -> skipped
    # entirely. A single sticker failing to send must not stop the rest of
    # the sequence, or the welcome flow that follows it.
    for sticker_id in _get_welcome_sticker_ids(welcome):
        try:
            sticker_msg = await message.reply_sticker(sticker_id)
            sent_ids.append(sticker_msg.id)
        except RPCError:
            logger.warning("Failed to send configured welcome sticker %s.", sticker_id, exc_info=True)

    # 2) WELCOME ANIMATION
    speed = welcome.get("animation_speed", "default")
    status = await _play_welcome_animation(message, speed)

    # 3) WELCOME MESSAGE (photo+caption or text), preserving spoiler state
    # and {mention}/{first}/{last}/{username}/{id} placeholders.
    ...
    if photo_id:
        ...
        sent = await message.reply_text(text, link_preview_options=LinkPreviewOptions(is_disabled=True))
        sent_ids.append(sent.id)
...
async def _handle_verification_start(client, message: Message, session_id: str, proof: str | None) -> None:
    """Resolves a `verify_<session_id>.<proof>` deep link — the browser
    bounce-back from our own /v/<session_id> landing route (see
    triss.services.shortener). A missing or incorrect `proof` is rejected
    outright regardless of timing; elapsed time alone is never treated as
    proof of completion."""
    user = message.from_user
    await db.upsert_user(user.id, user.username, user.first_name, user.last_name)
    ...
    if outcome == shortener.VerificationOutcome.PROVIDER_UNAVAILABLE:
        # Not the user's fault and retrying won't help until the owner fixes
        # the provider configuration — no retry keyboard, no failed-attempt
        # strike, no delivery. See triss/services/shortener.py.
        logger.error(
            "Verification session %s reached PROVIDER_UNAVAILABLE — the configured "
            "shortener provider cannot genuinely confirm completion. Nothing delivered.",
            session_id,
        )
        await message.reply_text(SHORTENER_UNAVAILABLE_TEXT)
        return
